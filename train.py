...
from utils.torch_utils import smart_DDP
...
from src.utils import creation_of_the_model
# os.environ["RANK"] = "8"  # Set RANK to 0
# os.environ["WORLD_SIZE"] = "8"
#
# # os.environ["MASTER_ADDR"] = "10.120.0.43"
...
def train(model, hyp, opt, device, callbacks):  # path/to/hyp.yaml or hyp dictionary
    (
        save_dir,
        epochs,
        batch_size,
    ...
    # Directories  runs/train/exp/weights
    ...
    for epoch in range(
        ...
        for i, (
            ...
            with amp.autocast(enabled=cuda):
                ...
                #     model = model.module
                #
                # # Check the device of each parameter
                # for name, param in model.named_parameters():
                #     print(f"Parameter '{name}' is on device: {param.device}")
                #
                # # Check the device of each buffer (e.g., running_mean, running_var in BatchNorm layers)
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--batch-size", type=int, default=7, help="total batch size for all GPUs"
    )
    parser.add_argument(
        "--imgsz",
        "--img",
...
def train_handler(model, opt, callbacks=Callbacks()):
    "previously called `main`"
    # Checks
    set_logging(RANK)
    if RANK in [-1, 0]:
    ...
    # Train
    if not opt.evolve:
        train(model, opt.hyp, opt, device, callbacks)
